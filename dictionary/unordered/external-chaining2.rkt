...
(library 
 ...
 (define (insert! table key val)
   ...
   (define assoc (make-assoc key (list val)))
   (define (merge-assocs assoc1 assoc2)
     (make-assoc(assoc-key assoc1) (append (assoc-value assoc1) (assoc-value assoc2))))
   (let insert-in-bucket 
     ...
     (cond 
       ...
       ((==? (assoc-key (car next)) key)
        (set-car! next (merge-assocs (car next) assoc)))
       (else
